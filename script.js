...
function combineResults(ruleRisk, aiProb) {

    const RULE_WEIGHT = 0.65;
    const AI_WEIGHT = 0.35;

    const ruleNormalized = ruleRisk / 100;
    const fused = (ruleNormalized * RULE_WEIGHT) + (aiProb * AI_WEIGHT);
...
async function analyzeDrawing() {
    ...
    try {
        ...
        if (aiProb !== null) {
            ...
            const fused = combineResults(result.risk, softenedProb);

            // อัปเดตเกจหลักด้วยคะแนนรวม (แทนคะแนน rule-based เดิม)
            ...
            const aiLabel = softenedProb >= 0.5 ? "Parkinson" : "Healthy";
            const aiConfidence = softenedProb >= 0.5 ? softenedProb * 100 : (1 - softenedProb) * 100;

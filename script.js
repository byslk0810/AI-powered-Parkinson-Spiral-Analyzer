...
async function analyzeDrawing() {
    ...
    try {
        ...
        window.lastCanvasSnapshot = canvas.toDataURL("image/png");   // ← เพิ่มบรรทัดนี้
...
if (navigator.canShare) {
    ...
    try {
        ...
        const file = new File([blob], "parkinson-spiral-result.png", { type: "image/png" });

        if (navigator.canShare({ files: [file] })) {

            const shareLang = localStorage.getItem("psa_lang") || "th";
            alert(shareLang === "en"
                ? "A share menu will appear. Please choose 'Save Image' to download your result. When finished, press the assessment button again."
                : "ระบบจะแสดงเมนูแชร์ กรุณาเลือก \"บันทึกรูปภาพ\" เพื่อดาวน์โหลดผลลัพธ์ เมื่อบันทึกรูปภาพเสร็จสิ้นให้กดแบบประเมินอีกครั้ง");
            await navigator.share({
                files: [file],
                title: "Parkinson Spiral Analyzer - ผลลัพธ์"
            });
            return;
        }
    } catch (err) {
        console.error("แชร์ผลลัพธ์ไม่สำเร็จ:", err);
    }
}

// fallback สำหรับเบราว์เซอร์ที่ไม่รองรับ Web Share API (เช่นคอม)
...
async function goToAssessment() {
    ...
    // กดครั้งที่ 2 ขึ้นไป (เซฟรูปไปแล้ว) — พาไปหน้าฟอร์มทันที
    if (hasSavedImage) {
        window.open(ASSESSMENT_FORM_URL, "_blank");
        return;
    }

    // กดครั้งแรก — เซฟรูปก่อน ยังไม่ไปหน้าฟอร์ม
    if (navigator.canShare) {

        try {
            const dataUrl = window.lastCanvasSnapshot || exportCanvasImage();
            ...
            if (navigator.canShare({ files: [file] })) {
                ...
                alert(lang === "en"
                    ? "A share menu will appear. Please choose 'Save Image' to download your result. When finished, press the assessment button again."
                    : "ระบบจะแสดงเมนูแชร์ กรุณาเลือก \"บันทึกรูปภาพ\" เพื่อดาวน์โหลดผลลัพธ์ เมื่อเสร็จสิ้นให้กดปุ่มแบบประเมินอีกครั้ง");
        ...
        // เบราว์เซอร์ที่ไม่รองรับ Web Share API (เช่นคอม) — เซฟแล้วไปฟอร์มในคลิกเดียว
        downloadDrawing();
        window.open(ASSESSMENT_FORM_URL, "_blank");
    }
